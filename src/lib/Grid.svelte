...
<script lang="ts">
  import type { ScrollBehavior } from 'svelte-virtual/types'
  ...
  export const scrollToPosition = (position: number, behavior: ScrollBehavior = scrollBehavior) => {
  ...
  const getItemProps = (index: number) => {
    ...
    return {
      rowIndex,
      columnIndex,
      style: `position: absolute; transform: translate3d(${columnIndex * itemWidth + marginLeft}px, ${
  ...
  $: if (offsetWidth || _columnCount) {
    indexes = getIndexes(itemCount, itemHeight, height, _columnCount, overScanColumn, _scrollPosition)
  }
  $: if (grid) {
...
>
  ...
  <div style:height="{innerHeight}px" style:width="100%">
    {#each indexes as index (getKey(index))}
      ...
      {:else}
        <slot name="placeholder" {index} {rowIndex} {columnIndex} {style}>Missing placeholder</slot>
      {/if}
    {/each}
  </div>
